...
#For detailes problem statement refer to "3 Ranking hospitals by outcome in a state" in ProgAssignment3.pdf in repo

#Logic Applied
#1. The fuction sources methods from function_best.R to get a subset of hospitals in a particular state 
...
rankhospital<-function(state, outcome, num='best'){
  ...
  if(num=='best'){
    result<-as.character(bestHospital_subset[1,1])
  }else if(num=='worst'){
    result<-as.character(bestHospital_subset[nrow(bestHospital_subset),1])
  }else{
    result<-as.character(bestHospital_subset[num,1])
  }
}